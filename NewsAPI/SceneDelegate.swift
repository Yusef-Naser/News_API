...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
      ...
      //  window.rootViewController = UINavigationController(rootViewController: SelectCountryVC())
        
        guard SharedData.instance.selectedCategory != nil ||
                (SharedData.instance.selectedCategory?.count ?? 0) > 0   else {
            window.rootViewController = SelectCategoryVC()
            self.window = window
        ...
        guard SharedData.instance.selectedCountry != nil ||
            ...
            window.rootViewController = SelectCountryVC()
            self.window = window
            window.makeKeyAndVisible()
            return
        }
